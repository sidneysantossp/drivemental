...
## Decisão arquitetural — Organizational Go-Live Readiness

O Arquiteto aceitou o PR protegido #22 como preparação documental do gate e confirmou que os checks pós-merge verdes não introduziram regressão técnica. O estado formal permanece: F7 RC técnico aprovado e promovido; beta técnico/controlado autorizado; go-live público/comercial bloqueado; mudanças funcionais ou metodológicas não autorizadas nesta etapa; Supabase antigo preservado como contingência; e o runbook F7-LEGACY-STATE-001 aceito sem manipulação da conta congelada.

Os três blockers continuam abertos. Para `F7.2-RETENTION-001`, o responsável competente deve fornecer decisões explícitas sobre controlador, encarregado/situação aplicável, bases legais por finalidade, retenção por categoria, backups/logs, fornecedores/transferências e canal de direitos; depois, a equipe técnica apenas reconcilia Privacy/Terms e executa uma prova sintética. Para `F7-BACKUP-PROD-001`, não repetir o backup se a execução reportada for válida; basta atestação sanitizada verificável, sem dump, senha, URL ou conteúdo do banco, e qualquer campo não realmente verificado deve permanecer `not_verified`. Para ownership/SLO, formalizar incident owner, escalation channel, log retention, objetivos de disponibilidade/fluxos/Edge Function e RPO/RTO, sem tratar os percentuais como SLA antes da aprovação.

A prova do canal de direitos deve ser um único caso sintético — solicitação, recebimento, triagem, verificação, encaminhamento, resposta e encerramento — somente após controlador e processo serem definidos. O fechamento público futuro seguirá: retenção/governança resolvida + backup verificado + ownership aprovado → Privacy/Terms publicados → nova sequência de release/testes/build/gates/runtime proof → decisão de go-live público. O projeto antigo permanece `READY FOR DECOMMISSION DECISION / DO NOT DECOMMISSION YET`.
